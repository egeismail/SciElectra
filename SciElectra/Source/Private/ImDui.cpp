...
static ImFloat4 ImHexToRGBAwA(ImUint rgba)
{
	static const ImUint sc_redShift = 24;
	static const ImUint sc_greenShift = 16;
	static const ImUint sc_blueShift = 8;
	static const ImUint sc_alphaShift = 0;

	static const ImUint sc_redMask = 0xff << sc_redShift;
	static const ImUint sc_greenMask = 0xff << sc_greenShift;
	static const ImUint sc_blueMask = 0xff << sc_blueShift;
	static const ImUint sc_alphaMask = 0xff << sc_alphaShift;

	ImFloat4 c;
	c.x = static_cast<float>((rgba & sc_redMask) >> sc_redShift) / 255.f;
	c.y = static_cast<float>((rgba & sc_greenMask) >> sc_greenShift) / 255.f;
	c.z = static_cast<float>((rgba & sc_blueMask) >> sc_blueShift) / 255.f;
	c.w = static_cast<float>((rgba & sc_alphaMask) >> sc_alphaShift) / 255.f;
...
namespace ImDui
{
	...
	GuiStyle::GuiStyle()
	{
		...
		TitleBarHeight			= 20.f;
		ResizeGripSize			= ImFloat2(10, 10);

		Colors[Color_Text]				= ImHexToRGBA(0xffffff);
		Colors[Color_Border]			= ImHexToRGBA(0x35005c);
		Colors[Color_WindowBg]			= ImHexToRGBAwA(0x2e002ecb);
		Colors[Color_WidgetBg]			= ImHexToRGBAwA(0xffffff21);
		Colors[Color_WidgetActive]		= ImHexToRGBA(0x4296FA);
		Colors[Color_TitleBar]			= ImHexToRGBA(0x2c77ff);
		Colors[Color_TitleBarCollapsed]	= ImHexToRGBA(0x2c92ff);
		Colors[Color_Slider]			= ImHexToRGBA(0x0055c2);
		Colors[Color_SliderActive]		= ImHexToRGBA(0x4a60b5);
		Colors[Color_Button]			= ImHexToRGBA(0x2C73D2);
		Colors[Color_ButtonHovered]		= ImHexToRGBA(0x0081CF);
		Colors[Color_ButtonActive]		= ImHexToRGBA(0x0089BA);
		Colors[Color_Collapse]			= ImHexToRGBA(0xFF9671);
		Colors[Color_CollapseHovered]	= ImHexToRGBA(0xFFC75F);
		Colors[Color_CollapseActive]	= ImHexToRGBA(0xF9F871);
		Colors[Color_ResizeGrip]		= ImHexToRGBAwA(0xB0A8B9ab);
		Colors[Color_ResizeGripHovered]	= ImHexToRGBAwA(0xBDC3C7ab);
		Colors[Color_ResizeGripActive]	= ImHexToRGBAwA(0xFFF6FFab);
		Colors[Color_TooltipBg]			= ImHexToRGBA(0x212121);
	}
